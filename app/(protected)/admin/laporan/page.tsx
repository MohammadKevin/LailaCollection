...
export default function ReportsPage() {
  ...
  const filteredSales = useMemo(() => {
    return sales.filter((sale) => {
      const matchSearch =
        sale.outlet?.name?.toLowerCase().includes(search.toLowerCase()) ||
        sale.cashier?.fullName?.toLowerCase().includes(search.toLowerCase());

      const matchDate = selectedDate
        ? new Date(sale.createdAt).toISOString().split("T")[0] === selectedDate
        : true;
  ...
  const handleExport = () => {
    const csvRows = [["Tanggal", "Outlet", "Kasir", "Payment", "Total"]];
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-6">
      <div className="space-y-6">
        <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-2">
              Monitoring penjualan, pengeluaran, profit dan stok produk
            </p>
          </div>

          <button
            onClick={handleExport}
            className="bg-slate-900 hover:bg-slate-800 transition-all text-white px-6 py-3 rounded-2xl font-semibold flex items-center justify-center gap-2"
          >
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-5 gap-5">
          <div className="bg-white rounded-2xl p-6 border border-slate-200">
            ...
            <p className="text-xs uppercase font-semibold tracking-wide text-slate-400 mt-5">
              Total Penjualan
            </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 grid md:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 rounded-2xl py-3 pl-11 pr-4 border border-slate-200 outline-none focus:ring-2 focus:ring-slate-300"
            />
          </div>
                ...
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="text-right px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-400">
                        Total
                      </th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {filteredSales.map((sale) => (
                      <tr key={sale.id} className="border-t border-slate-100">
                        ...
                        <td className="px-6 py-5">
                          <div className="inline-flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full text-xs font-semibold text-slate-700">
                ...
                <div className="space-y-4">
                  {products.slice(0, 5).map((product) => (
                    ...
                    >
                      ...
                      <div className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-full text-xs font-semibold">
                        Stock {product.stock}
                      </div>
                    </div>
                  ))}
